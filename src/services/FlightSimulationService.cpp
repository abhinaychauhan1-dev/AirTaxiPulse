...
FlightSimulationService::FlightSimulationService(QObject *parent)
    : QObject(parent)
    , m_phase(0.0)
    , m_flightModeLabel(QStringLiteral("VTOL Hover"))
    , m_telemetry()
{
    ...
    appendHistory(m_fpvHistory, qAbs(m_telemetry.track() - m_telemetry.heading()) + 1.0);
    appendHistory(m_fpvHistory, qAbs(m_telemetry.track() - m_telemetry.heading()) + 0.6);
    appendHistory(m_fpvHistory, qAbs(m_telemetry.track() - m_telemetry.heading()));
...
double FlightSimulationService::casProgress() const { return clamp((m_telemetry.cas() - 100.0) / 80.0, 0.0, 1.0); }
double FlightSimulationService::altProgress() const { return clamp((m_telemetry.altBaro() - 7600.0) / 1200.0, 0.0, 1.0); }
double FlightSimulationService::vsProgress() const { return clamp(qAbs(m_telemetry.vs()) / 1600.0, 0.0, 1.0); }
double FlightSimulationService::attitudeProgress() const { return clamp((qAbs(m_telemetry.pitch()) + qAbs(m_telemetry.roll())) / 12.0, 0.0, 1.0); }
double FlightSimulationService::headingProgress() const { return clamp(1.0 - qAbs(m_telemetry.track() - m_telemetry.heading()) / 30.0, 0.0, 1.0); }
double FlightSimulationService::fpvProgress() const { return clamp(1.0 - qAbs(m_telemetry.track() - m_telemetry.heading()) / 30.0, 0.0, 1.0); }
...
QString FlightSimulationService::vsTrendLabel() const { return m_telemetry.vs() >= 0.0 ? QStringLiteral("Climb trend") : QStringLiteral("Descent trend"); }
...
void FlightSimulationService::updateSimulation()
{
    ...
    if (cyclePosition < 8.0) {
        ...
        m_flightModeLabel = QStringLiteral("Vertical Climb");
        ...
        m_flightModeLabel = QStringLiteral("Transition to Forward Flight");
        ...
        m_flightModeLabel = QStringLiteral("Cruise");
        ...
        m_flightModeLabel = QStringLiteral("Approach & Descent");
